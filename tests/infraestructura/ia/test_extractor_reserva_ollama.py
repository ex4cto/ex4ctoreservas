"""Tests for ExtractorReservaOllama — maps DatosExtraidos → ContextoVenta.

Uses a mock of the ExtractorIA port (not ExtractorOllama directly).
"""
...
@pytest.fixture()
def extractor(mock_extractor_ia: MagicMock) -> ExtractorReservaOllama:
    return ExtractorReservaOllama(extractor_ia=mock_extractor_ia)
...
class TestExtractorReservaOllama:
    def test_mapeo_completo(
        self, extractor: ExtractorReservaOllama, mock_extractor_ia: MagicMock
    ) -> None:
        ...
        with patch("garay.infraestructura.ia.extractor_reserva_ollama.os.unlink"):
    ...
    def test_servicio_nombre_es_ignorado(
        ...
        # Check all str fields — none should contain "Carlos"
        for field_val in [
            ctx.cliente_nombre,
    ...
    def test_adultos_none_devuelve_none(
        ...
        """Scenario 6 — adultos=None → ctx.adultos == None (photo AI did not detect a value)."""
    ...
    def test_temp_file_eliminado_cuando_extractor_lanza_excepcion(
        ...
        with patch("garay.infraestructura.ia.extractor_reserva_ollama.os.unlink") as mock_unlink:
