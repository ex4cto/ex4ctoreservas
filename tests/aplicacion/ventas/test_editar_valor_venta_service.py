"""Tests for EditarValorVentaService — Phase 2.6.2 RED."""
...
def _make_cmd(
    motivo: str = "ajuste precio",
    nuevo_valor_venta: Dinero = Dinero(400),
) -> object:
    from garay.aplicacion.ventas.comandos import EditarValorVentaComando

    return EditarValorVentaComando(
        venta_id=uuid.uuid4(),
...
def _make_service(
    ...
    return EditarValorVentaService(
        ventas=ventas,
        auditoria=auditoria,
        reglas_repo=reglas,
        puntos_repo=puntos,
...
class TestEditarValorVentaService:
    def test_importable(self) -> None:
        from garay.aplicacion.ventas.editar_valor_venta import EditarValorVentaService

        assert EditarValorVentaService is not None
    ...
    def test_happy_path_calls_cambiar_valor_venta(self) -> None:
        venta = _make_venta_mock()
        ...
        cmd = _make_cmd(nuevo_valor_venta=Dinero(400))
        service.ejecutar(cmd)  # type: ignore[union-attr]
        venta.cambiar_valor_venta.assert_called_once_with(Dinero(400))
    ...
    def test_happy_path_audit_record_has_editar_valor_venta(self) -> None:
        venta = _make_venta_mock()
        ventas, auditoria, reglas, puntos, comisiones = _make_repos(venta=venta)
        motor = _make_motor()
        service = _make_service(ventas, auditoria, reglas, puntos, comisiones, motor)
        ...
        assert saved.accion == AccionAuditoria.EDITAR_VALOR_VENTA

    def test_domain_error_propagated(self) -> None:
        venta = _make_venta_mock()
        venta.cambiar_valor_venta.side_effect = ValorVentaMenorQueAbono("blocked")
        ventas, auditoria, reglas, puntos, comisiones = _make_repos(venta=venta)
        service = _make_service(ventas, auditoria, reglas, puntos, comisiones, _make_motor())
        cmd = _make_cmd()
    ...
    def test_mismo_valor_venta_propagated(self) -> None:
        venta = _make_venta_mock()
        venta.cambiar_valor_venta.side_effect = MismoValorVenta("idempotent")
        ventas, auditoria, reglas, puntos, comisiones = _make_repos(venta=venta)
        service = _make_service(ventas, auditoria, reglas, puntos, comisiones, _make_motor())
        cmd = _make_cmd()
    ...
    def test_neto_supera_nuevo_valor_propagated(self) -> None:
        venta = _make_venta_mock()
        venta.cambiar_valor_venta.side_effect = NetoIgualOSuperaValorVenta("blocked")
        ventas, auditoria, reglas, puntos, comisiones = _make_repos(venta=venta)
        service = _make_service(ventas, auditoria, reglas, puntos, comisiones, _make_motor())
        cmd = _make_cmd()
    ...
    def test_motor_calcular_referido_zero(self) -> None:
